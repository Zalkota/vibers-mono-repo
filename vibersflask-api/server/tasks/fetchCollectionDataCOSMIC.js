...
async function runTask(){
    ...
    for(let tokenId=0; tokenId<totalSupply; tokenId+=1){
        ...
        let URI = `https://ipfs.io/ipfs/QmcCsHvjMsCRRjG9YPLB1XepNoUVXoG5h3L93Yea8srd4w/${tokenId}`
        console.log(URI)
    ...
    if(writeTraitsFile)
    {
        fs.writeFileSync( path.join ( `./server/output/${collectionName}.json` ) , JSON.stringify( traitsMap ) );
    }
...
async function  downloadImage(tokenId, url){
   ...
   //     || tokenId == 2841) return
    ...
    let image_path = path.join ( `./server/output/images/${collectionName}/${tokenId}.jpg` )
