    ...
    export default class ApplicationManager  {
        ...
        constructor(   ){
            ...
            static async generateNewApplicationForUser(publicAddress, mongoInterface)
            {
                ...
                let newApp = await mongoInterface.getAPIApplicationModel().insertOne( {publicAddress: publicAddress, applicationId: appId } )
            ...
            static async findAllApplicationsForUser(publicAddress, mongoInterface){
                ...
                return await mongoInterface.getAPIApplicationModel().find( {publicAddress: publicAddress } )
 
            }

            static async findApplicationById(appId, mongoInterface){
                return await mongoInterface.getAPIApplicationModel().find( {applicationId: appId } )
            ...
            static async findRecentLoggedRequestCountForUser( publicAddress, mongoInterface ){
                ...
                let matchingEpochCountersArray = await mongoInterface.getAppEpochCounterModel().find( {publicAddress: publicAddress, epochHour: { $lte: maxEpochHour, $gt: minEpochHour } })
