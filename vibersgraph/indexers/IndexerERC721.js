...
module.exports =  class IndexerERC721 {
    ...
    if(existingEntry){
        await mongoInterface.updateOne('erc721_tokens', {tokenId: tokenId, contractAddress: contractAddress}, {accountAddress: accountAddress , lastUpdatedAt: Date.now()  } )
    }else{
        await mongoInterface.insertOne('erc721_tokens', {tokenId: tokenId, contractAddress: contractAddress, accountAddress: accountAddress , lastUpdatedAt: Date.now()  }   )
    }
    ...
    static async recordTransfer( from, to ,contractAddress , tokenId , blockNumber, transactionIndex ,mongoInterface){
        ...
        await mongoInterface.insertOne('erc721_transfers', {from:from, to:to, tokenId: tokenId, contractAddress: contractAddress,   blockNumber: blockNumber, transactionIndex: transactionIndex, createdAt: Date.now()  }   )
