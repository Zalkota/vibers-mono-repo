            ...
            <!-- <div>
              ...
              <div class="my-16 text-center">
                ...
                <div  class="flex flex-row">
                  ...
                  <div class="select-none bg-blue-500 hover:bg-orange-300 p-2 px-8 rounded border-gray border-2 cursor-pointer" @click="mint"> Mint </div>
                  <div class="flex-grow"></div>
              </div> 
              <br>
                <div class="text-white text-xs">
...
*/  
          ...
          async fetchTokenURI(){
             ...
             const nftContract = this.web3Plug.getCustomContract( ERC721ABI, contractData.cosmicCaps.address  )
          ...
          async mint( ){
            ...
            let userAddress = this.web3Plug.getActiveAccountAddress()
             ...
             let ethValue = parseInt(amt) * 6 * 100000020000000000 ; 
            ...
            const overrides = {
                ...
                gasLimit: Math.floor(
                    200000 * amt -
                        ((200000 * amt) / 100) * (amt - amt * 0.2),
                ).toString(),
            };

            const nftContract = this.web3Plug.getCustomContract( ERC721ABI, contractData.cosmicCaps.address  )

            // this.totalSupply = await nftContract.methods.mint( userAddress, amt ).send({from: userAddress, value: ethValue })
            this.totalSupply = await nftContract.methods.mint( userAddress, amt ).send({from: userAddress, value: ethValue })
